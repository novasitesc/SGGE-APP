...
export async function GET(req: Request) {
  try {
    ...
    if (animalId && !isUuid(animalId)) return jsonError("animalId inválido.");
    ...
    if (referencia) q = q.ilike("referencia", `%${referencia}%`);
    if (animalId) q = q.eq("registro_id", animalId);
    ...
    if (error?.message.includes("historial_sistema")) {
      let lq = admin
        ...
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);
      if (referencia) lq = lq.ilike("arete", `%${referencia}%`);
      if (animalId) lq = lq.eq("animal_id", animalId);
